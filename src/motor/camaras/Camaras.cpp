...
Camara::Camara(int x, int y, int w, int h, SDL_Renderer &view) {
  ...
  width = w;
  height = h;
  pos_mundo = {x, y};                  // posicion de el mundo
  pos_centro = {x + w / 2, y + h / 2}; // es la posicion de la camara
  printf("pos_mundo: %d, %d\n", pos_mundo.x, pos_mundo.y);
  printf("pos_centro: %d, %d\n", pos_centro.x, pos_centro.y);
  printf("w,h: %d, %d\n", w, h);
