...
void Plotter::render_linea(SDL_Renderer *r, Coordenadas p1, Coordenadas p2,
                           SDL_Color color) {
...
void Plotter::rellenoscandline(SDL_Renderer *r, Figura *fig) {
  ...
  for (int y = ymin + 1; y < ymax; ++y) {
    for (int i = 0; i < numvert; ++i) {
      bool td = tiene_determinante({xmin, y}, {xmax, y}, vertices[i],
                                   vertices[(i + 1) % numvert]);
      if (td) {
        Coordenadas ip = interseccion({xmin, y}, {xmax, y}, vertices[i],
                                      vertices[(i + 1) % numvert]);
  ...
  for (int v = 0; v < isize / 2; ++v)
    get().render_linea(r, inter[isize - v - 1], inter[v],
                       fig->get_rellenocolor());
};

void Plotter::pintar_textura(SDL_Renderer *r, SDL_Texture *textura,
...
void Plotter::pintar_textura_flip(SDL_Renderer *r, SDL_Texture *textura,
...
void Plotter::set_renderer(SDL_Renderer &r) { get().render = &r; };
