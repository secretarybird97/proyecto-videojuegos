...
Pipeline::Pipeline(SDL_Renderer &ren) { ptrren = &ren; };

void Pipeline::puntos(std::vector<Coordenadas> p, SDL_Color c) {
  // cada p es un punto en el espacio, simplemente llamar a plotter para
  // pintarlos
  for (auto &punto : p)
...
void Pipeline::figuras(std::vector<Figura *> f) {
  ...
  for (auto &fig : f) {
    ...
    for (int j = 0; j < size; j++) {
      Plotter::get().render_linea(ptrren, vertices[j], vertices[(j + 1) % size],
                                  fig->get_strokecolor());
    }
  }
};
...
void Pipeline::pintar_texturas(Sprite *obj) {
  if (!obj->get_flip())
    Plotter::get().pintar_textura(
        ptrren, obj->get_textura(), *obj->get_sprite_src(),
        *obj->get_sprite_frame(), obj->get_sprite_sizes());
  else
    Plotter::get().pintar_textura_flip(
        ptrren, obj->get_textura(), *obj->get_sprite_src(),
        *obj->get_sprite_frame(), obj->get_sprite_sizes());
};

void Pipeline::pintar_texturas(Tile *obj) {
  Plotter::get().pintar_textura(ptrren, obj->get_textura(), *obj->get_src(),
                                *obj->get_dst(), obj->get_sizes());
};
